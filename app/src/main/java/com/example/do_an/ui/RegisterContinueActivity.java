...
public class RegisterContinueActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        actionBar.hide();
        String Username = getIntent().getStringExtra("PHONE_NUMBER");
        ...
        btdkyREcnt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (edMK.getText().toString().isEmpty() || edMK2.getText().toString().isEmpty()) {
                    Toast.makeText(RegisterContinueActivity.this, "Chưa nhập đủ thông tin", Toast.LENGTH_SHORT).show();
                } else if (!edMK.getText().toString().equals(edMK2.getText().toString())) {
                    Toast.makeText(RegisterContinueActivity.this, "Mật khẩu xác nhận không đúng", Toast.LENGTH_SHORT).show();

                } else if (edMK.getText().toString().length() < 6 || edMK2.getText().toString().length() < 6  || !edMK.getText().toString().matches("^[0-9]+$")) {
                    Toast.makeText(RegisterContinueActivity.this, "Mật khẩu phải là 6 chữ số", Toast.LENGTH_SHORT).show();
                    ...
                    Intent intent = new Intent(RegisterContinueActivity.this, LoginActivity.class);
                ...
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(RegisterContinueActivity.this, "Lưu thành công", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(RegisterContinueActivity.this, task.getException().getMessage(), Toast.LENGTH_SHORT).show(); // Khi lưu thất bại
                        }
                    }
                }).addOnFailureListener(new OnFailureListener() { // Khi lưu thất bại
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(RegisterContinueActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
                ...
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            ...
                            Toast.makeText(RegisterContinueActivity.this, "Đăng ký thành công", Toast.LENGTH_LONG).show();
                            ...
                            Toast.makeText(RegisterContinueActivity.this, "Đăng ký thất bại", Toast.LENGTH_LONG).show();
                        }
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(RegisterContinueActivity.this, "Đăng ký thất bại", Toast.LENGTH_SHORT).show();
                    }
                });
    }
