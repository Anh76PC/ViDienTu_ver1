...
public class WithdActivity extends AppCompatActivity {
    TextView soduviRT, soduviXem, iddataRT;
    Button btnt;
    ImageButton backRT;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soduviRT = findViewById(R.id.soduviRT);
        soduviXem = findViewById(R.id.soduviXem);
        backRT = findViewById(R.id.backRT);
        getInfo(phoneNumber);
        btnt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                String price = soduviRT.getText().toString();
                if(soduviRT.getText() == null)
                    Toast.makeText(WithdActivity.this, "Chưa nhập số tiền cần rút", Toast.LENGTH_SHORT).show();
                else{
                    int sodu = Integer.parseInt(String.valueOf(soduviRT.getText()));
                    updateToFireStore(phoneNumber,sodu);
                    updateNotification(phoneNumber,iddata, price, date, hour);
                }
            }
        });
        backRT.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                finish();
            }
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (document.exists()) {
                        ...
                        if(currentSoDu < sodu)
                            Toast.makeText(WithdActivity.this, "Số tiền rút lớn hơn số dư", Toast.LENGTH_SHORT).show();
                        else {
                            ...
                            userRef.update("soDuVi", newSoDu)
                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if (task.isSuccessful()) {
                                                Toast.makeText(WithdActivity.this, "Rút tiền thành công", Toast.LENGTH_SHORT).show();
                                                numberOfNotifications++;
                                            } else {
                                                Toast.makeText(WithdActivity.this, "Lỗi khi rút tiền: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                            }
                                        }
                                    });
                        }
                    } else {
                        Toast.makeText(WithdActivity.this, "Không tìm thấy người dùng", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(WithdActivity.this, "Lỗi khi truy cập dữ liệu: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    private void updateNotification(String phoneNumber, String titletran, String pricetran, String date, String hour){
        ...
        transactionInfoDocRef.collection("RutTien").add(newTransactionMap)
                .addOnSuccessListener(documentReference -> {
                    Toast.makeText(WithdActivity.this, "Giao dịch đã được thêm vào Firebase", Toast.LENGTH_SHORT).show();
                })
                .addOnFailureListener(e -> {
                    Toast.makeText(WithdActivity.this, "Lỗi khi thêm giao dịch vào Firebase: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
                finish();
