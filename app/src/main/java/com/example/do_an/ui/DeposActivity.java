...
public class DeposActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        backNT = findViewById(R.id.backNT);
        getInfo(phoneNumber);
        btnT.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(soduviNT.getText() == null)
                    Toast.makeText(DeposActivity.this, "Chưa nhập số tiền cần nạp", Toast.LENGTH_SHORT).show();
                else{
                    int sodu = Integer.parseInt(String.valueOf(soduviNT.getText()));
                    if(sodu < 10000)
                        Toast.makeText(DeposActivity.this, "Nạp tối thiểu 10.000đ", Toast.LENGTH_SHORT).show();
                    else
                    {
        ...
        backNT.setOnClickListener(new View.OnClickListener() {
            @Override
                        ...
                        userRef.update("soDuVi", newSoDu)
                                .addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(DeposActivity.this, "Nạp tiền thành công", Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(DeposActivity.this, "Lỗi khi nạp tiền: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                        }
                                        finish();
                                    }
                                });
                    } else {
                        Toast.makeText(DeposActivity.this, "Không tìm thấy người dùng", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(DeposActivity.this, "Lỗi khi truy cập dữ liệu: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    private void updateNotification(String phoneNumber, String titletran, String pricetran, String date, String hour) {
        ...
        transactionInfoDocRef.collection("NapTien").add(newTransactionMap)
                .addOnSuccessListener(documentReference -> {
                    Toast.makeText(DeposActivity.this, "Giao dịch đã được thêm vào Firebase", Toast.LENGTH_SHORT).show();
                })
                .addOnFailureListener(e -> {
                    Toast.makeText(DeposActivity.this, "Lỗi khi thêm giao dịch vào Firebase: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
    }
